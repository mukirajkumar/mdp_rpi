...
def msg_passing(msg): #pass in the message that u read frm andriod 
    if len(msg) != 0:
        ...
        if message[0] == 'STM': #Encoded 
            ...
            interfaces[STM].write(stm_msg) #STM = 0 so this is calling interfaces[0] = STM() so its like calling STM().write() whr rpi will write/send the msg to STM-polymorphism 
...
if __name__ == '__main__':
    ...
    interfaces = []
    interfaces.append(STM()) #I think its something like new STM() -polymorphism
    ...
    #args is the arg that u passed in to the target function (readMsg) 
    #Overhere the process created is to read the msg frm andriod
    android = Process(target=readMsg, args=(queue, interfaces[AND])) #target = the function that u wanna run
...
#     stm = Process(target=readMsg, args=(queue, interfaces[STM]))
    ...
    # Establish connections between RPi and all other interfaces - STM(), Algo() and Andriod()
    #Need to establis connection first so that afterwards u can perform read() and write()
...
#     stm.start()
    ...
    try:
        while True:
            ...
            print("Now passing on the message........")
            msg = msg_passing(msg) #Andriod pass msg frm andriod to STM
